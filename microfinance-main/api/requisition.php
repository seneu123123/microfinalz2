...
error_log("Session status: " . (isset($_SESSION['user_id']) ? "User ID: " . $_SESSION['user_id'] : "No session"));
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        error_log("Processing GET request for user: " . $_SESSION['user_id']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        // Simple insert
        $stmt = $pdo->prepare("INSERT INTO requisitions (user_id, request_date, remarks) VALUES (?, NOW(), ?)");
        $stmt->execute([$_SESSION['user_id'], $input['remarks'] ?? '']);
